#!/bin/bash

##
# Demonstrates a multi-statement transaction with a rollback
#
# See http://developer.marklogic.com/blog/marklogic_is_acid_compliant_nosql
#

HOSTNAME=10.211.55.11
PORT=8000
USER=admin
PASSWORD=admin

# create the transaction and get the location
printf "Creating a new Transaction:"
TX_ID=$(curl --anyauth --user ${USER}:${PASSWORD} -X POST -d "" -i \
  -s -H "Content-type: text/plain" \
  http://${HOSTNAME}:${PORT}/v1/transactions | tr -d '\r' | sed -En 's/^Location: \/v1\/transactions\/(.*)/\1/p')
printf " ${TX_ID}\n"

echo "Inserting Doc 1"
curl --anyauth --user ${USER}:${PASSWORD} -X PUT -d '{"hello":"mars"}' -i \
  -H "Content-type: application/json" \
  "http://${HOSTNAME}:${PORT}/v1/documents?uri=/hello-mars.json&txid=${TX_ID}"

echo "Inserting Doc 2"
curl --anyauth --user ${USER}:${PASSWORD} -X PUT -d '{"goodbye":"mars"}' -i \
  -H "Content-type: application/json" \
  "http://${HOSTNAME}:${PORT}/v1/documents?uri=/goodbye-mars.json&txid=${TX_ID}"

echo ""
echo "Ask for Documents before commit"
curl --anyauth --user ${USER}:${PASSWORD} -X GET -i \
  -H "Accept: multipart/mixed; boundary=document-part-boundary" \
  "http://${HOSTNAME}:${PORT}/v1/documents?uri=/hello-mars.json&uri=/goodbye-mars.json"

echo ""
printf "Rollback Transaction: ${TX_ID}"

curl --anyauth --user ${USER}:${PASSWORD} -X POST -d "" -i \
  -H "Content-type: text/plain" \
  -s --fail --show-error \
  "http://${HOSTNAME}:${PORT}/v1/transactions/${TX_ID}?result=rollback"
if [[ $? != 0 ]] ; then
  echo "error on commit"
  exit 1
fi
printf " ...Done.\n"

echo ""
echo "Ask for Documents after commit"
curl --anyauth --user ${USER}:${PASSWORD} -X GET -i \
  -H "Accept: multipart/mixed; boundary=document-part-boundary" \
  "http://${HOSTNAME}:${PORT}/v1/documents?uri=/hello-mars.json&uri=/goodbye-mars.json"
